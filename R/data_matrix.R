...
#' @param covariates Optional matrix of spatial covariates with one row per quadrature point
#'        (both data and dummy points for the focal cell). Columns represent different covariates.
#'        If provided, these will be appended to the design matrix after dispersion features.
#' @param verbose Logical flag for verbosity (default: FALSE)
#'
#' @return A list containing the design matrix (`data`) with an intercept, dispersion features,
#'         and optional covariates; and the response vector (`response`) indicating data (1) vs. dummy (0) points.
#' @export
make_data <- function(Q,
  ...
  if (!is.null(covariates)) {
    ...
    if (!is.matrix(covariates) && !inherits(covariates, "Matrix")) {
      covariates <- as.matrix(covariates)
    }

    # Check dimensions
    expected_rows <- length(response)
    if (nrow(covariates) != expected_rows) {
      stop(sprintf("covariates must have %d rows (one per quadrature point for focal cell), but has %d rows",
                   expected_rows, nrow(covariates)))
    }

    # Check for missing values
    if (any(is.na(covariates))) {
      stop("covariates cannot contain missing values (NA)")
    }

    # Check for constant covariates (will cause numerical issues)
    constant_cols <- apply(covariates, 2, function(x) length(unique(x)) == 1)
    if (any(constant_cols)) {
      warning(sprintf("Covariate(s) %s are constant and may cause numerical issues",
                      paste(which(constant_cols), collapse = ", ")))
    }
  ...
  # Build the design matrix by combining intercept, dispersion features, and optional covariates
  if (is.null(covariates)) {
    design_matrix <- cbind(beta0 = 1, dispersions)
  } else {
    design_matrix <- cbind(beta0 = 1, dispersions, covariates)
  }
