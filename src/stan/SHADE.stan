...
model {
  if(num_pt_groups > 0) {
    for(j in 1:num_pot) {
      ...
      // normal prior with different widths for different potential functions
    ...
    to_vector(beta_global[1,:]) ~ normal(mean_alpha,tau_alpha_global[1]);
		sigma_beta_indiv ~ normal(1.5,scale_sigmas);
  ...
  for (s in 1:n_samples) {
	...
	tau_alpha_indiv ~ normal(1,scale_sigmas);
  sigma_beta_local ~ normal(1,scale_sigmas);
  tau_alpha_local ~ normal(1,scale_sigmas);
